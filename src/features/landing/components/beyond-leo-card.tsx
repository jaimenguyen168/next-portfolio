...
export function BeyondLeoCard({ visible, onClose }: Props) {
  return (
    <Dialog open={visible} onOpenChange={(v) => !v && onClose()}>
      <DialogContent
        showCloseButton={false}
        className="w-full max-w-sm p-0 gap-0 border-0 overflow-hidden"
        style={{
      ...
      >
        ...
        >
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(to top, rgba(8,12,36,0.97) 0%, transparent 100%)",
            }}
          />
          <span
        ...
        <div className="p-4 flex flex-col gap-3">
          ...
          <p className="text-sm text-slate-400 leading-relaxed">
            Ruled by the Sun, Leo burns bright. Confident, generous, and
            magnetic by nature, Leos are built to lead — not from ego, but from
            a genuine desire to lift others up. They carry warmth wherever they
            go, and the people around them feel it.
          </p>
          <p className="text-sm text-slate-400 leading-relaxed">
            Leo is a fixed sign, which means once committed, they don&apos;t
            waver. That same steadiness that makes them fiercely loyal can also
            make them resistant to change. But their passion is rarely
            performative — it comes from somewhere real.
          </p>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {["Stubborn", "Needs validation", "Dramatic", "Inflexible"].map(
                (s) => (
                  <span
                    key={s}
